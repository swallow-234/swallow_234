...
class SimilarImageApp:
    ...
    def load_deleted_records(self):
        ...
        self.hash_cache_file = os.path.join(folder_path, "hash_cache.txt") # 設定哈希快取檔案路徑
    ...
    def create_widgets(self):
        ...
        self.sort_option = ttk.Combobox(top_frame, values=["感知哈希 (pHash)", "差值哈希 (dHash)", "平均哈希 (aHash)", "檔案像素", "檔案大小", "修改日期", "建立日期", "讀取日期", "檔案名稱", "色相", "隨機"], width=15)
        ...
        tk.Button(action_frame_row2, text="上一組", command=self.show_prev_pair).pack(side=tk.LEFT, padx=5, expand=True)
        tk.Button(action_frame_row2, text="執行刪除並關閉程式", command=self.execute_deletion).pack(side=tk.LEFT, padx=5, expand=True)
        tk.Button(action_frame_row2, text="下一組", command=self.show_next_pair).pack(side=tk.LEFT, padx=5, expand=True)
    ...
    def find_similar_images(self):
        ...
        if os.path.exists(self.hash_cache_file): # 清除舊的哈希快取
        ...
        if total_files == 0:
            self.progress_label['text'] = "資料夾中沒有可用的圖片。"
            tk.messagebox.showinfo("提示", "資料夾中沒有可用的圖片。")
            return

        self.total_batches = (total_files + self.batch_size - 1) // self.batch_size
        
        if "哈希" in sort_by:
            self.progress_label['text'] = "開始計算哈希值..."
            self.root.update_idletasks()
            self.hash_algorithm_to_use = self.get_hash_algorithm_from_name(sort_by)
            self.root.after(100, self.process_next_hash_batch)
        else:
            self.root.after(100, self.process_next_ssim_batch)

    def get_hash_algorithm_from_name(self, name):
        if name == "感知哈希 (pHash)":
            return imagehash.phash
        elif name == "差值哈希 (dHash)":
            return imagehash.dhash
        elif name == "平均哈希 (aHash)":
            return imagehash.average_hash
        return None

    def process_next_hash_batch(self):
        start_index = self.current_batch_index * self.batch_size
        
        if self.current_batch_index >= self.total_batches:
            self.progress_label['text'] = "所有哈希值都已計算完畢。開始比對..."
            self.root.update_idletasks()
            self.find_similar_by_hash_algorithm()
            return
        ...
        hash_algorithm = self.hash_algorithm_to_use
        
        with open(self.hash_cache_file, 'a', encoding='utf-8') as f_hash:
            for i, path in enumerate(current_batch_paths):
                try:
                    with Image.open(path) as img:
                        h = hash_algorithm(img)
                        f_hash.write(f"{os.path.basename(path)},{str(h)}\n")
                except Exception as e:
                    print(f"無法為圖片 {os.path.basename(path)} 計算哈希值: {e}")
            
                self.progressbar['value'] = (self.current_batch_index * self.batch_size + i + 1) / (self.total_batches * self.batch_size) * 100
                self.progress_label['text'] = f"處理批次 {self.current_batch_index + 1}/{self.total_batches}: {i+1}/{len(current_batch_paths)}"
                self.root.update_idletasks()

        self.current_batch_index += 1
        self.root.after(100, self.process_next_hash_batch)

    def find_similar_by_hash_algorithm(self):
        """從快取檔案讀取所有哈希值並分組比對"""
        self.similar_pairs = []
        hashes = {}
        ...
        if os.path.exists(self.hash_cache_file):
            with open(self.hash_cache_file, 'r', encoding='utf-8') as f:
                lines = f.readlines()
                self.progress_label['text'] = f"讀取 {len(lines)} 個哈希值，準備比對..."
                self.root.update_idletasks()

                for line in lines:
                    try:
                        filename, hash_str = line.strip().split(',')
                        if hash_str not in hashes:
                            hashes[hash_str] = []
                        hashes[hash_str].append(filename)
                    except ValueError:
                        print(f"無效的哈希快取行: {line.strip()}")
        
        # 找到哈希值相同的圖片
        for hash_str, filenames in hashes.items():
            if len(filenames) > 1:
                # 將找到的重複圖片組成多個圖片對
    ...
    def process_next_ssim_batch(self):
        start_index = self.current_batch_index * self.batch_size
        ...
        for file_type, names_list in grouped_by_type.items():
            ...
            for i in range(len(names_with_size)):
                for j in range(i + 1, len(names_with_size)):
                    ...
                    self.progressbar['value'] = (self.current_batch_index * self.batch_size + progress_count) / (self.total_batches * self.batch_size * 0.5) * 100 if self.total_batches > 0 else 100
    ...
    def update_details_table(self, file_a, file_b, similarity):
        ...
        # 檢查並插入每一行，如果值不同則使用 'different' 標籤
        data = [
            ("相似度", f"{similarity:.4f}", f"{similarity:.4f}"),
            ("解析度 (像素)", details_a['resolution'], details_b['resolution']),
            ("檔案大小", details_a['size'], details_b['size']),
            ("檔案類型", details_a['type'], details_b['type']),
            ("修改日期", details_a['mod_date'], details_b['mod_date']),
            ("建立日期", details_a['creation_date'], details_b['creation_date']),
            ("檔案名稱(重新命名)", os.path.basename(file_a), os.path.basename(file_b))
        ]

        for detail, value_a, value_b in data:
            if value_a != value_b:
                tags = ('different',)
            else:
                tags = ()
            
            # 特殊處理 '檔案名稱' 欄位，它需要 'editable' 標籤
            if detail == "檔案名稱(重新命名)":
                tags = tags + ('editable',)

            self.tree.insert("", "end", values=(detail, value_a, value_b), tags=tags)
